// Variables
const id = document.getElementById("id"); // ID membre
...
const modifier = document.getElementById("modifier"); // Bouton "Modifier"
const annuler = document.getElementById("annuler"); // Bouton "Annuler"
...
// Récupère les données du membre grâce à son ID et à l'API
fetch(
  "./php/Stagiaire/lire_un.php?id_membre=" +
    window.location.search.split("?id=")[1]
)
  .then((response) => response.json())
  .then((response) => {
    // Rentre les données du membre dans le formulaire
    id.value = response["id_membre"];
    prenom.value = response["login_membre"];
    nom.value = response["nom_membre"];
...
modifier.addEventListener("click", (e) => {
  ...
  if (testPrenom() && testNom()) {
    ...
    fetch("./php/Stagiaire/modifier.php", {
      method: "PUT",
      headers: {
